...
if (isset($_GET['restaurantid'])) {
   $q5="SELECT `taste`,`environment`,`service`,`text`,`customer_id`,`customer_name`, `review_id` , `restaurantId` FROM `review` r,`customer` c WHERE  r.`customerId`=c.`customer_id` AND `restaurantId`=".$_GET['restaurantid']." ORDER BY `post_time`";
 } elseif (isset($_SESSION['restaurant_id'])){
 $q5="SELECT `taste`,`environment`,`service`,`text`,`customer_id`,`customer_name`, `review_id` , `restaurantId` FROM `review` r,`customer` c WHERE  r.`customerId`=c.`customer_id` AND `restaurantId`=".$_SESSION['restaurant_id']." ORDER BY `post_time`";
